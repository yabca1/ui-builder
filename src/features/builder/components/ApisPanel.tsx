...
export function ApisPanel() {
  ...
  const renderSchemaFieldEditor = (field: SchemaField, index: number, type: "request" | "response") => (
    <div key={`${type}-${index}`} className="grid grid-cols-[minmax(0,1fr)_96px_auto_28px] gap-1.5 items-center bg-white dark:bg-slate-950/60 p-2 rounded-lg border border-slate-200 dark:border-slate-850">
      <input
        value={field.name}
        onChange={(e) => handleSchemaFieldChange(type, index, "name", e.target.value)}
        placeholder={type === "request" ? "user.firstName" : "data.id"}
        className="min-w-0 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1.5 text-xs outline-none text-slate-900 dark:text-slate-100"
      />
      <select
        value={field.type}
        onChange={(e) => handleSchemaFieldChange(type, index, "type", e.target.value as SchemaFieldType)}
        className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1.5 text-xs outline-none text-slate-900 dark:text-slate-100"
      >
        {schemaFieldTypes.map((fieldType) => (
          <option key={fieldType} value={fieldType}>{fieldType}</option>
        ))}
      </select>
      <label className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 dark:text-slate-400 select-none">
        <input
          type="checkbox"
      ...
      <button
        ...
        className="size-7 rounded-md text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 cursor-pointer font-black"
      >
        ×
      </button>
    </div>
  ...
  return (
    <div className="flex h-full flex-col min-h-0 text-slate-700 dark:text-slate-350 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-colors duration-150">
      {/* Header Tabs */}
      <div className="flex shrink-0 border-b border-slate-100 dark:border-slate-850 p-2.5 gap-1.5 select-none bg-slate-50/50 dark:bg-slate-950/20">
        <button
          type="button"
          onClick={() => { setActiveSubTab("integrations"); setEditingIntId(null); setEditingPathId(null); }}
          className={`flex-1 rounded-xl py-2 px-3 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer ${
        ...
        {activeSubTab === "credentials" && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">API Credentials</h3>
              {!editingCredId && (
                <button
                  type="button"
                  onClick={handleStartNewCred}
                  className="rounded-md bg-teal-600 hover:bg-teal-700 text-white px-2 py-1 text-xs font-bold cursor-pointer transition"
                >
                  + Add
                </button>
              )}
            ...
            {editingCredId ? (
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-3.5 flex flex-col gap-3">
                ...
                <label className="flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                  ...
                  <input
                    ...
                    className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2.5 py-1.5 text-xs outline-none text-slate-900 dark:text-slate-100"
                  />
                </label>
                <label className="flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                  Secret Value
                ...
                <div className="flex justify-end gap-2 mt-1">
                  <button
                    ...
                    className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="rounded-md bg-indigo-600 text-white px-2.5 py-1.5 text-xs font-bold cursor-pointer"
                  >
                    Save
                  </button>
                </div>
            ...
            <div className="flex flex-col gap-2">
              {credentials.length === 0 ? (
                ...
                credentials.map(c => (
                  <div key={c.id} className="group flex items-center justify-between border border-slate-200 dark:border-slate-800 rounded-xl p-3 bg-white dark:bg-slate-950 shadow-sm hover:border-slate-300 dark:hover:border-slate-700 transition">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="rounded p-1 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-900 cursor-pointer"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="rounded p-1 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/20 cursor-pointer"
                      >
                        Delete
                      </button>
                    </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between select-none">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-450 dark:text-slate-500">Service Integration</h3>
                </div>
                <div className="flex gap-1.5">
                  <button
                    type="button"
                    onClick={() => importInputRef.current?.click()}
                    className="rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-905 px-2.5 py-1.5 text-[11px] font-bold cursor-pointer transition flex items-center gap-1"
                  >
                  ...
                  {!editingIntId && (
                    <button
                      ...
                      className="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white px-2.5 py-1.5 text-[11px] font-bold cursor-pointer transition flex items-center gap-1"
                    >
                      <svg className="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
            ...
            {editingIntId ? (
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-3.5 flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  <label className="flex-1 flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                    Authentication
                    <select
                      value={intAuthType}
                ...
                {intAuthType !== "none" && (
                  <div className="flex flex-col gap-2 bg-white dark:bg-slate-950/60 p-2.5 rounded-lg border border-slate-200 dark:border-slate-850">
                    <label className="flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                      Link Credential
                      {credentials.length === 0 ? (
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between select-none">
                    <span className="text-[10px] font-bold text-slate-400">Default Headers</span>
                    <button
                      type="button"
                  ...
                  {intHeaders.map((header, idx) => (
                    <div key={idx} className="flex gap-1.5 items-center">
                      <input
                        ...
                        className="flex-1 rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1 text-xs outline-none text-slate-900 dark:text-slate-100"
                      />
                      <input
                        placeholder="Value"
                        value={header.value}
                ...
                <div className="flex justify-end gap-2 mt-2">
                  {editingIntId !== "new" && (
                    <button
                      ...
                      className="mr-auto rounded-md bg-rose-50 text-rose-600 border border-rose-100 dark:bg-rose-950/20 dark:border-rose-900/30 px-2.5 py-1.5 text-xs font-bold cursor-pointer"
                    >
                      Delete
                    </button>
                  )}
            ...
            {selectedIntegrationId && !editingIntId && (
              <div className="flex flex-col gap-4 border-t border-slate-100 dark:border-slate-850 pt-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">API Operations / Paths</h3>
                  {!editingPathId && (
                    <button
                      type="button"
                      onClick={handleStartNewPath}
                      className="rounded-md bg-teal-600 hover:bg-teal-700 text-white px-2.5 py-1 text-xs font-bold cursor-pointer"
                    >
                      + Add Path
                    </button>
                  )}
                    ...
                    {pathsForActiveIntegration.length === 0 ? (
                      ...
                      pathsForActiveIntegration.map((p) => (
                        <button
                          ...
                          className={`flex items-center justify-between border rounded-xl p-3 text-left transition select-none ${
                            selectedPathId === p.id
                              ? "border-indigo-500 dark:border-indigo-600 bg-indigo-50/10 dark:bg-indigo-950/10"
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-[10px] text-slate-450 font-mono mt-0.5">
                              <span className={`mr-1.5 font-bold ${
                                p.method === "GET" ? "text-emerald-600" : p.method === "POST" ? "text-blue-600" : "text-amber-600"
                              }`}>{p.method}</span>
                              {p.path}
                            </span>
                          </div>
                ...
                {editingPathId ? (
                  <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 p-3.5 flex flex-col gap-3">
                    ...
                    <div className="flex gap-2">
                      <label className="w-24 shrink-0 flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                        Method
                        <select
                          value={pathMethod}
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-800 pt-3 flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Request Schema / Parameters</span>
                        ...
                        <button
                          ...
                          className="rounded-md bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 px-2 py-1 text-[10px] font-bold cursor-pointer"
                        >
                          + Field
                        </button>
                      </div>
                      {pathRequestSchema.length === 0 ? (
                        <div className="text-center py-2 text-[10px] text-slate-400 font-medium bg-white dark:bg-slate-950 rounded border border-slate-200 dark:border-slate-850">No request fields defined.</div>
                      ) : (
                        <div className="flex flex-col gap-1.5">
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-800 pt-3 flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Response Definition</span>
                        <span className="text-[9px] text-slate-400 font-semibold italic">Manual editing disabled</span>
                      </div>
                      {pathResponseSchema.length === 0 ? (
                        <div className="text-center py-2 text-[10px] text-slate-400 font-medium bg-white dark:bg-slate-950 rounded border border-slate-200 dark:border-slate-850">No response fields defined. Run "Test API" to infer automatically.</div>
                      ) : (
                        <div className="flex flex-col gap-1.5">
                          {pathResponseSchema.map((field) => (
                            <div key={field.name} className="flex justify-between items-center bg-slate-50 dark:bg-slate-950/60 p-2.5 rounded border border-slate-200 dark:border-slate-850 text-xs">
                ...
                {selectedPathId && !editingPathId && activePath && (
                  <div className="flex flex-col gap-4 border-t border-slate-100 dark:border-slate-850 pt-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-black text-slate-800 dark:text-slate-200">Test API Console</h4>
                      <button
                        type="button"
                    ...
                    {/* Inputs */}
                    <div className="flex flex-col gap-3 bg-slate-50/50 dark:bg-slate-950/20 rounded-xl p-3.5 border border-slate-200 dark:border-slate-800">
                      <span className="text-[10px] font-bold text-slate-400 select-none">API Request Parameters</span>
                      {activePath.requestSchema?.length === 0 ? (
                        <div className="text-[10px] text-slate-400 font-semibold py-1">No request parameters defined. You can trigger directly.</div>
                      ) : (
                        activePath.requestSchema.map((field) => {
                          ...
                          return (
                            <label key={field.name} className="flex flex-col gap-1 text-[10px] font-bold text-slate-400">
                              <span>
                                {field.name}
                                {field.required && <span className="text-rose-500 font-black ml-0.5">*</span>}
                                <span className="ml-1 font-mono font-semibold text-slate-400">({field.type})</span>
                              </span>
                              {field.type === "boolean" ? (
                                <select
                                  value={String(value)}
                                  onChange={(e) => updateValue(e.target.value)}
                                  className="rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1 text-xs outline-none text-slate-900 dark:text-slate-100"
                                >
                                ...
                                <textarea
                                  ...
                                  rows={3}
                                  className="resize-y rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1 text-xs outline-none text-slate-900 dark:text-slate-100 font-mono"
                                />
                              ) : (
                                <input
                                  value={value}
                      ...
                      <button
                        ...
                        disabled={isTesting}
                        className="rounded-lg bg-teal-600 hover:bg-teal-700 disabled:bg-teal-400 text-white py-2 text-xs font-bold cursor-pointer transition flex items-center justify-center gap-1.5"
                      >
                    ...
                    {/* Output */}
                    {testResult && (
                      <div className="flex flex-col gap-3 border border-slate-200 dark:border-slate-800 rounded-xl p-3 bg-white dark:bg-slate-950 shadow-inner">
                        <div className="flex items-center justify-between text-xs font-bold">
                          <span className="text-slate-400">Response Console</span>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
                            testResult.success ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20" : "bg-rose-50 text-rose-600 dark:bg-rose-950/20"
                          }`}>
                        ...
                        {testResult.success && testResult.data && (
                          <button
                            ...
                            className="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white py-1.5 text-xs font-bold transition flex items-center justify-center cursor-pointer select-none"
                          >
                        ...
                        {testResult.duration !== undefined && (
                          <div className="text-[10px] text-slate-400 font-medium">
                            Duration: <span className="font-bold text-slate-600 dark:text-slate-300">{testResult.duration}ms</span>
                          </div>
                        )}

                        {testResult.validationErrors && testResult.validationErrors.length > 0 && (
                          <div className="rounded bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 p-2 text-[10px] text-rose-600 dark:text-rose-400">
                            <span className="font-bold block mb-0.5">Validation Errors:</span>
                        ...
                        {testResult.data !== undefined && (
                          <pre className="rounded bg-slate-50 dark:bg-slate-900 p-2 text-[10px] text-slate-700 dark:text-slate-300 overflow-x-auto max-h-48 font-mono select-text">
                        ...
                        {testResult.logs && testResult.logs.length > 0 && (
                          <div className="border-t border-slate-100 dark:border-slate-850 pt-2 flex flex-col gap-1">
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Internal Trace Logs</span>
                            <pre className="text-[9px] text-slate-500 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50 p-2 rounded overflow-x-auto font-mono whitespace-pre-wrap select-text leading-relaxed">
